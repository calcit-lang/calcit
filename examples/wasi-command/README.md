...
这个示例展示 Calcit 当前可直接用于小型批处理业务的最短路径：读取命令行参数和环境变量，从 Wasmtime 显式预开放的目录读取文本，转换后写回文件，并用稳定的进程状态码报告失败。它使用默认的 WASI Preview 1 路径；WASI 0.3.1 `--boundary component` 已支持参数、环境变量和退出码，但尚未覆盖文件能力。
