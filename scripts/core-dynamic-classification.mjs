...
function classify(entry) {
  ...
  if (entry.intent === "intentional-macro-syntax") {
    return {
      owner: "macro-system",
      decision: "retain-reviewed",
      rationale: "宏语法随展开阶段变化；保留显式开放边界，并通过专项测试验证语义。",
    };
  }
  if (namespace === "calcit.internal") {
    return {
      ...
      decision: "retain-reviewed",
      rationale: "编译器自有的 trait/impl 元数据不是公开值契约；保留可审阅的边界说明。",
    };
  }
...
const lines = [
  ...
  "# 内置 core 的 Dynamic 边界清单",
  "",
  `清单包含 **${rows.length}** 个 schema-Dynamic 位置，分布于 **${report.data.summary.definitions}** 个定义。`,
  "",
  `当前待迁移位置：**${migrationCount}** 个。`,
  "",
  "每个位置都标明所属子系统与迁移决策。`retain-reviewed` 表示已审阅的开放边界，不代表类型正确性豁免；清单用于审阅，不按位置数量判定类型正确性。`migrate` 标记需要优先收敛的调用方可见契约。",
  "",
  "## 汇总",
  "",
  "| 决策 / 所属子系统 | 位置数 |",
