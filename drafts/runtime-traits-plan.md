...
- [ ] **`assert-trait` 运行时检查**：前移错误发现时机
  - 语法：`(assert-trait x Show)` 或 `(requires x Show)`
...
1. **立即开始**：`defimpl` 宏 + `assert-trait` 检查
   - 这两个功能用户需求高，实现相对独立
   - 可以显著改善开发体验
...
- [x] ~~`assert-trait` 运行时检查与错误消息格式~~ → 短期优先
