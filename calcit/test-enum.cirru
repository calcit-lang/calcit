...
{} (:about "|Machine-generated snapshot. AI AGENTS: never edit this file directly — changes will be overwritten on recompile. Inspect via `cr query`; modify via `cr edit` / `cr tree`. MANDATORY first step: run `cr docs agents --full`.") (:package |test-enum)
  ...
  :files $ {}
    |test-enum.main $ %{} :FileEntry
      :defs $ {}
        ...
        |main! $ %{} :CodeEntry (:doc |)
          :code $ quote
            defn main! () $ do (println "|Testing enum runtime validation...") (test-enum-creation) (test-tag-match-validation) (test-tuple-to-enum) (println "|All tests passed!")
          :examples $ []
          :schema $ :: :fn
            {} (:return :unit)
              :args $ []
        ...
        |test-tuple-to-enum $ %{} :CodeEntry (:doc "|Test automatic tuple-to-enum rewrite")
          :code $ quote
            defn test-tuple-to-enum () $ do (println "|Testing tuple-to-enum rewrite...")
              ; Untyped tuple :: :ok gets rewritten to %:: Result0 :ok
