    ...
    'test-edn.main $ %{} 'FileEntry
      :defs $ {}
        ...
        'DemoEnum $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defenum DemoEnum (:ok) (:err 'String)
          :examples $ []
          :schema $ :: 'EnumDef
        ...
        'Response $ %{} 'CodeEntry (:doc |)
          ...
          :schema $ :: 'StructDef
        'Team $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defstruct Team
            :members $ :: 'List Person
          :examples $ []
          :schema $ :: 'StructDef
        'main! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn main! () (log-title "|Testing edn") (test-edn) (test-edn-comment)
        ...
        'test-edn $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn test-edn ()
            let
                edn-demo "|%{} :Person (:age 23) (:name |Chen)"
              ; "no stable order"
              assert=
                count $ to-lispy-string $ %{} Person (:name |Chen) (:age 23)
              ...
              assert= "|{} $ :code $ quote (+ 1 2 3)" $ trim $ format-cirru-edn
                {} $ :code $ :: 'quote ([] |+ |1 |2 |3)
              assert= "|{} $ :code $ quote (+ 1 2 3)" $ trim $ format-cirru-edn
                {} $ :code $ cirru-quote (+ 1 2 3)
              ...
              assert=
                ...
                , "|{} (|D 4) (|Z 3) (|a 1) (|b 2)"
              assert=
                trim $ format-cirru-edn $ {} (:c 2) (:a1 0)
                  :b $ [] 1 2
                  :a 1
                , "|{} (:a 1) (:a1 0) (:c 2)\n  :b $ [] 1 2"
              assert= "|:: '&core-list-methods $ [] 1 2 3" $ trim $ format-cirru-edn
                :: &core-list-methods $ [] 1 2 3
              assert= "|:: 'test" $ trim $ format-cirru-edn (:: :test)
              assert= "|:: 'test :a :b" $ trim $ format-cirru-edn (:: :test :a :b)
              let
                  enum-ok $ parse-cirru-edn "|%:: :DemoEnum :ok" $ {} (:DemoEnum DemoEnum)
                assert= :ok $ &enum:nth enum-ok 0
                assert= "|%:: 'DemoEnum 'ok" $ trim $ format-cirru-edn enum-ok
              let
                  enum-err $ parse-cirru-edn "|%:: :DemoEnum :err |oops" $ {} (:DemoEnum DemoEnum)
        ...
        'test-runtime-map-decode $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn test-runtime-map-decode ()
            let
        ...
        'test-top-level-typed-edn $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn test-top-level-typed-edn ()
            assert=
        ...
        'test-typed-edn $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn test-typed-edn ()
            let
