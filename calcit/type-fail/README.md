...
- `collection-member-contract-mismatch.cirru` 会验证已知 collection receiver 将 `get` / `contains?` 的索引或键、`includes?` 的成员、`assoc` 的索引/键/值，以及 `dissoc` 的所有 rest 索引或键类型带到调用点，包括 Enum 的 Number payload index；它也覆盖原生 `&map:dissoc`、`&list:concat` 与 `&merge` 的同质 variadic contract，`filter` / `any?` / `every?` / `each` / `map` 对 List/Set 成员或 Map pair callback 的输入与返回契约，`foldl` / `reduce` 的 accumulator/member reducer 关系，`sort` / `&list:sort` 的 `T, T -> Number` comparator 关系，以及函数形式 `&list:sort-by` 的 `T -> K` selector 关系。
