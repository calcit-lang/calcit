...
{}
  ...
  :files $ {} $ 'app.main
    %{} 'FileEntry
      :defs $ {}
        'clock-fixed-main! $ %{} 'CodeEntry
          :doc "|用确定性的 WASI 假宿主验证时钟编号与纳秒到毫秒的换算。"
          ...
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
          :tags $ #{} :time :wasi
        'clock-main! $ %{} 'CodeEntry
          :doc "|输出系统时钟与单调时钟，用于验证 WASI capability wiring。"
          ...
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
          :tags $ #{} :time :wasi
        'clocks-valid? $ %{} 'CodeEntry
          :doc "|验证系统时钟为正数，且单调时钟在同一进程内不会倒退。"
          ...
          :tests $ [] $ %{} 'TestEntry (:name |valid-readings)
            ...
            :tags $ #{} :core :time :unit :wasi :wasm
        'exit-7! $ %{} 'CodeEntry
          :doc "|以状态码 7 终止进程，用于验证 command 退出边界。"
          :code $ quote $ defn exit-7! () (quit! 7)
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
          :tags $ #{} :control :wasi
        'exit-invalid! $ %{} 'CodeEntry
          :doc "|以越界状态码终止进程，用于验证各 command backend 拒绝隐式取模。"
        ...
        'filesystem-absolute-main! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn filesystem-absolute-main! ()
            if
              filesystem-read-error? |/workspace/input.txt
              println "|WASI-filesystem-absolute: ok"
              quit! 1
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
        'filesystem-denied-main! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn filesystem-denied-main! ()
            if
              filesystem-read-error? |workspace/input.txt
              println "|WASI-filesystem-denied: ok"
              quit! 1
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
        'filesystem-invalid-utf8-main! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn filesystem-invalid-utf8-main! ()
            if
              filesystem-read-error? |workspace/invalid.txt
              println "|WASI-filesystem-invalid-utf8: ok"
              quit! 1
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
          ...
          :code $ quote $ defn filesystem-main! ()
            let
                ...
                output-ok? $ match output
                  (:ok _) true
                  (:err _) false
              if (and input-ok? output-ok?)
                println "|WASI-filesystem: ok"
                quit! 1
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
        'filesystem-read-dir-error-main! $ %{} 'CodeEntry
          :doc "|通过 WASI 假宿主验证畸形目录记录仍返回 Result :err。"
          :code $ quote $ defn filesystem-read-dir-error-main! ()
            if
              filesystem-read-dir-error? |workspace/listing
              println "|WASI-read-dir-error: ok"
              quit! 1
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
          :tags $ #{} :file :wasi
        'filesystem-read-dir-error? $ %{} 'CodeEntry
          :doc "|验证目录枚举失败仍以 Result :err 表达。"
          ...
          :tests $ [] $ %{} 'TestEntry
            ...
            :code $ quote $ assert= true
              filesystem-read-dir-error? |/calcit-wasi-filesystem-does-not-exist
            :tags $ #{} :file :unit :wasi
        'filesystem-read-dir-main! $ %{} 'CodeEntry
          :doc "|通过真实与假 WASI 宿主验证目录分页、UTF-8 名称和确定排序。"
        ...
        'filesystem-traversal-main! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn filesystem-traversal-main! ()
            if
              filesystem-read-error? |workspace/../secret.txt
              println "|WASI-filesystem-traversal: ok"
              quit! 1
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
        'main! $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn main! () (println |WASI-stdout: "|你好") (echo |WASI-echo) (eprintln |WASI-stderr: 42)
            println |WASI-env: $ option:unwrap-or
        ...
        'needs-arg $ %{} 'CodeEntry (:doc |)
          ...
          :tests $ [] $ %{} 'TestEntry (:name |returns-input)
            ...
            :tags $ #{} :core :unit :wasi :wasm
        'random-error? $ %{} 'CodeEntry
          :doc "|验证越界的安全随机请求返回 String 错误。"
          ...
          :tests $ []
            %{} 'TestEntry
              ...
              :tags $ #{} :core :crypto :unit :wasi :wasm
            %{} 'TestEntry
              :name |rejects-oversized-length
              :code $ quote $ assert= true (random-error? 65537)
              :tags $ #{} :core :crypto :unit :wasi :wasm
            %{} 'TestEntry
              :name |rejects-fractional-length
              :code $ quote $ assert= true (random-error? 1.5)
              :tags $ #{} :core :crypto :unit :wasi :wasm
        'random-fixed-main! $ %{} 'CodeEntry
          :doc "|由确定性 WASI 假宿主验证四字节随机请求。"
          :code $ quote $ defn random-fixed-main! ()
            if (random-success? 4)
              println "|secure-random-fixed: ok"
              quit! 1
          :examples $ []
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
          :tags $ #{} :crypto :wasi
        'random-main! $ %{} 'CodeEntry
          :doc "|验证安全随机 API 的成功与越界 Result 语义。"
          ...
          :schema $ :: 'Fn $ {} (:return 'Unit)
            :args $ []
          :tags $ #{} :crypto :wasi
        'random-success? $ %{} 'CodeEntry
          :doc "|验证指定长度的安全随机请求返回 Buffer。"
