...
- Rust 测试覆盖普通 tag 拒绝与 dot 保留；Calcit 的 trait 正常路径由现有 `calcit/test-traits.cirru` 验证。负向编译诊断无法通过正常执行的 definition `:tests` 表达，留在预处理器测试。全量 Rust 测试需要允许本地 HTTP fixture bind；sandbox 内的两项权限失败在授权环境复跑通过。
