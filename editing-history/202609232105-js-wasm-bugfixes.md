...
- PR #1297 review 指出 Enum payload 内的 Map/Set 仍会落到指针比较。补上递归 Map/Set 分派和 Set 深比较，并用正反 WASM fixture 覆盖。`calcit.core/=` 测试也增加同 variant、不同 payload 的断言。
...
- PR #1297 review found that Map/Set values nested inside Enum payloads still fell back to pointer comparison. Dispatch them recursively, add deep Set equality, and cover both equal and unequal cases in WASM fixtures. The `calcit.core/=` test also checks a same-variant payload mismatch.
