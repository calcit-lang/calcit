...
- JS fixture 明确使用 Component Interface IR 的确定性 enum 顺序映射 Canonical ABI discriminant，避免调用端与宿主端以相同错误编号产生假阳性。
...
- Use the Component Interface IR's deterministic enum ordering for Canonical ABI discriminants explicitly in the JavaScript fixture so matching mistakes on both sides cannot create a false positive.
