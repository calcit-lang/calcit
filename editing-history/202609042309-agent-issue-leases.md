...
- `release` 先原子删除权威 lock ref，再更新 Issue 标签；完成镜像更新后会重新读取 lock ref，若期间已有新 claim，则用新 lease 元数据把 Issue 修复回 `agent:claimed`。若标签更新中断，后续 claim 仍可修复无锁的陈旧 `agent:claimed` 状态。
