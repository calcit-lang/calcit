...
- Calcit `:tests` 覆盖直接、宿主与嵌套 Result 往返语义；Rust 只补充 ABI layout、memory 边界和拒绝路径。
...
- Cover direct, host, and nested-Result round trips in Calcit `:tests`; keep Rust coverage focused on ABI layout, memory boundaries, and rejection paths.
