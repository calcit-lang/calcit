...
- 按已加载的 `CalcitTypeAnnotation::TypeRef` 与 resolved trait bound 改写 schema，明确区分 nominal reference、泛型 TypeVar 与内建类型，不用 EDN 文本匹配猜测类型含义。
...
- Rewrite schemas from loaded `CalcitTypeAnnotation::TypeRef` values and resolved trait bounds so nominal references remain distinct from generic type variables and built-in types instead of relying on EDN text matching.
