...
- Measured sequential delta after the standalone caps cutover: binary targets `3 → 2` and direct dependencies remain `30`. An isolated release build on the change's original base kept the `calcit` binary byte-identical at `9,515,920` bytes; removing a separate runner target does not change runtime payload.
