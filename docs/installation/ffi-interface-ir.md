...
V3 会忽略 `:kind :external-object` 的 typed host capability trait：它用于静态
...
V3 表示 `Unit`、`Bool`、`Number`、`String`、`Buffer`、同质 `List`、明确的
`Option` / `Result`，以及由 namespace-qualified declaration 支撑的本地 Struct/Enum。
此外，V3 将 `Int8`、`UInt8`、`Int16`、`UInt16`、`Int32`、`UInt32`、`Int64`、
`UInt64`、`Float32` 与 `Float64` 分别导出为独立的 `kind`。普通 `Number` 仍导出为
`number`，不得根据值、名称或调用位置猜测整数宽度。数值 refinement 在参数、结果、
Struct field、Enum payload、List、Option 与 Result 中使用同一递归表示，并自然进入 revision。
...
monomorphic. It also exports every width-specific integer and floating-point
refinement as its own `kind`; `Number` remains `number` and is never narrowed
by guessing. Only declarations transitively reachable from an FFI signature
...
callbacks because v3 does not define their ownership, thread-affinity, or
...
当前 v3 只导出带有效 lowering 字段的本地 raw binding，忽略 snapshot 中普通
...
untyped `DynFn` callback，因为 v3 尚未定义 ownership、线程亲和性与生命周期契约。
