...
```bash
calcit edit cp app.main/demo --from '3.2' --path '4' --at after
```
...
- 含义：定义级迁移；若只是同命名空间重命名，使用 `calcit edit rename`。
- 适合：整理命名或模块边界。

## 3) `wrap`：给目标套一层结构

```bash
calcit tree wrap app.main/demo --path '@5.2' --code 'quote (when cond self)'
```
...
```bash
calcit tree raise app.main/demo --path '@5.2.1'
```
...
```bash
calcit tree rewrite app.main/demo --path '@5.2' --with self=. --code 'quote (-> self normalize emit)'
```
