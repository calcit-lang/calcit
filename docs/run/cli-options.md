...
`calcit wasm --boundary component` 复用同一 WASM 入口，输出供 Component tooling 包装的 core module，不增加 component/WIT 顶层命令。当前 adapter 接受 `defwasm-import` / `defwasm-export` 的 `Bool`、`Buffer`、`Number`、UTF-8 `String` 与递归同质 `List<T>` 参数和结果：Bool 使用严格限制为 `0`/`1` 的 Canonical ABI `i32`，Number 使用 `f64`；Buffer 映射到 WIT `list<u8>`，String 映射到 WIT `string`；List 只接受闭合、单态且 item type 已受支持的 schema，可组合 `Bool`、`Buffer`、`Number`、`String` 和 List。Buffer、String 与 List 的 core 参数都使用 `(i32 ptr, i32 len)`，但 lift 后保持不同的 Calcit 类型。对应 export 结果返回指向 `(ptr,len)` return area 的 `i32`，import 结果使用 caller-provided return area；非法 Bool、长度溢出和越界区间直接 trap。模块只保留显式声明的 Component imports，同时导出 `memory` 与可按需增长 memory 的 `cabi_realloc`，且不继承 native core target 的隐式 `math/io` imports。WIT 生成与 runnable component packaging 仍由 `calcit-bindgen` 负责。
