...
WASI command 中的 `try-parse-cirru-edn-as` 与 `format-cirru-edn` 直接使用编译器已经推导出的闭合类型，不在运行时探测值类型。当前除标量、标量 `List` 和标量 `Map` 外，也支持顶层 nominal Struct 的标量字段往返：输入使用 Cirru EDN 的 `%{} 'TypeName (:field value)`，字段顺序可以不同，但结构名、字段集合和字段类型必须与声明完全一致；重复、缺失、未知字段和数值越界都会返回 `Result :err`。格式化按声明字段顺序生成 canonical Cirru EDN。嵌套容器字段、嵌套 Struct 与 Enum 仍会在 codegen 阶段明确拒绝，不会退回 `Dynamic` 或生成近似数据。
