...
`calcit wasm --boundary component` 复用同一 WASM 入口，输出供 Component tooling 包装的 core module，不增加 component/WIT 顶层命令。当前 adapter 接受 `defwasm-import` / `defwasm-export` 的 `Unit` 结果、`Bool`、`Buffer`、`Number`、明确宽度数值、UTF-8 `String`、递归同质 `List<T>`、闭合单态 `Option<T>` / `Result<T,E>`、monomorphic Struct record 与闭合单态 Enum variant：Bool 使用严格限制为 `0`/`1` 的 Canonical ABI `i32`，Number 使用 `f64`，`Int8` 至 `UInt64` 与 `Float32` / `Float64` 使用对应的 `i32`、`i64`、`f32`、`f64` flat shape 和 memory layout；Buffer 映射到 WIT `list<u8>`，String 映射到 WIT `string`；List、Struct field 与 Enum payload 都必须形成只包含受支持类型的闭合 schema。Option/Result 只认类型推导保留的 `calcit.core` 名义类型，共用 Canonical ABI variant layout 与 flat join；普通 Enum 的 case 顺序来自声明，Struct 字段顺序来自规范化 schema。旧 `Optional<T>`、Dynamic、anonymous/open Enum 和用户自定义同名 Option/Result 不会被猜测转换。Unit 返回零个 ABI result，参数中的 Unit 必须直接省略。超过单结果上限的同步 export 结果返回 return-area 指针，import 结果使用 caller-provided return area；非法 Bool/discriminant、数值越界或精度损失、内部 enum tag/count、长度溢出和越界区间直接 trap。显式 `:async true` 的 export 使用 WASI 0.3 async core shape，无 core result，并通过 `calcit:component/canonical` 下的 `task-return/<export-symbol>` import 完成一次返回；当前只覆盖直接运行到完成的 export，async import 会以 `E_COMPONENT_ABI_ASYNC_IMPORT_UNSUPPORTED` 拒绝。模块同时导出 `memory` 与可按需增长 memory 的 `cabi_realloc`，且不继承 native core target 的隐式 `math/io` imports。WIT 生成、canonical builtin wiring 与 runnable Component packaging 仍由 `calcit-bindgen` 负责。
