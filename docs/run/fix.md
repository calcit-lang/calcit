...
避免嵌套改写提前移动 source path。Cirru EDN preview 会在 `:filters :preset-id` 返回 preset 名称，并在
`:filters :expanded-rule-ids` 返回实际执行的规则，Agent 无需依赖人类日志猜测范围：
...
Cirru EDN stdout 是一个完整 value，包含 `:schema-version`、`:command`、Snapshot `:revision`、`:filters`、`:validation`、`:suggestions`、`:diagnostics`
和 `:next`。每条 suggestion 携带 Snapshot `:source-file`、stable rule ID、diagnostic code、表层 definition/path、subtree fingerprint、
...
`:filters :expanded-rules` 进一步说明每条实际规则的 `:evidence-source`、`:diagnostic-code`、`:lifecycle` 和
