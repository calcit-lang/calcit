...
JSON stdout 是一个完整 value，包含 `schema_version`、`command`、Snapshot `revision`、filters、suggestions、diagnostics
和 `next`。每条 suggestion 携带 stable rule ID、diagnostic code、表层 definition/path、subtree fingerprint、quoted AST
的 original/replacement 与 applicability。日志和 command echo 只写 stderr，Agent 不需要解析人类文本或生成的 JS。
...
写入流程复用 `tree replace` 的 `--expect` guard 和现有 transaction：全部 replacement 先在 staged Snapshot 上执行，重新
加载并预处理选定 scope，最后才原子替换源文件。revision 过期、节点不匹配、替换重叠、parse/schema/preprocess 失败时均不写入。
