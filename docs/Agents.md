...
# 输出情况：
# - 文件不存在 → 提示 "No .calcit-error.cirru file found"（黄色）
# - 文件为空 → 提示 "Error file is empty (no recent errors)"（绿色，表示编译成功）
# - 有错误 → 显示完整错误堆栈和位置信息（红色）
```
...
- ⚡ **快速反馈**：只处理修改的部分，无需重新加载整个程序
- 🎯 **精确控制**：明确指定哪些定义发生了变化
- 🔍 **错误隔离**：通过 `cr query error` 快速查看特定修改引起的错误
- 🔄 **保持状态**：watcher 持续运行，保持程序运行时状态（适用于有状态的应用）
