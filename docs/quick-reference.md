...
- `FsPath .read-text`、`.read-dir`、`.walk-dir`、`.write-text`：返回 `Result` 的可恢复文件效果；WASI 0.3 Component command 目前仅支持 preopen 内最多 4 MiB UTF-8 文本读取，其他文件效果待实现；core WASM 不可用
