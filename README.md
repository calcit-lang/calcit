...
| --- | --- |
| [`calcit-bindgen`](https://github.com/calcit-lang/calcit-bindgen) | 独立的实验性生成器；已经负责确定性的 Rust/Calcit/TypeScript/WIT generation、manifest、compatibility diff 与 stale check。0.14.20 支持的同步类型子集可生成 `component.wasm`，并已通过 Wasmtime 与 jco/Node 验证；Struct/Enum、resource、async 等更广 Component packaging 仍在推进。core 只保留 Interface IR export/schema/conformance；已完成的 generator 拆分由 [#544](https://github.com/calcit-lang/calcit/issues/544) 追踪。 |
