...
/// Check if a function name looks like it was generated by a macro or compiler
/// These typically contain special characters like %, $, or other markers
fn is_generated_function_name(name: &str) -> bool {
  // Check for common patterns in generated function names
  name.contains('%') || name.contains('$') || name.starts_with("__") || name.is_empty()
}
...
fn check_recur_arity_in_expr(
  ...
  match expr {
    ...
    Calcit::Fn { info, .. } => {
      // Skip checking for macro-generated functions (e.g., from loop macro)
      if is_generated_function_name(info.name.as_ref()) {
        return;
      }
...
pub fn preprocess_defn(
  ...
  match (args.first(), args.get(1)) {
    (
      ...
      // 2. Macro-generated functions (containing %, $, etc.) - like loop macro
      // 3. calcit.core functions - external library, should be fixed separately
      let is_core_ns = ctx.file_ns == calcit::CORE_NS;
      if !has_marked_args && !is_generated_function_name(def_name.as_ref()) && !is_core_ns {
