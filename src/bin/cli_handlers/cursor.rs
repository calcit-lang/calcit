...
fn cursor_last_query_from_edn(value: &Edn) -> Result<CursorLastQuery, String> {
  ...
  Ok(CursorLastQuery {
    ...
    source: optional_string(&entry, "source")?.unwrap_or_else(|| "all".to_owned()),
...
fn cursor_last_query_to_edn(query: &CursorLastQuery) -> Edn {
  Edn::map_from_iter([
    ...
    (Edn::tag("source"), Edn::str(query.source.as_str())),
...
mod tests {
  ...
  fn cursor_state_round_trips_as_cirru_edn() {
    ...
    let path = vec![fixture.main_tail, 1];
  ...
  fn persisted_cursor_moves_after_insertion_before_it() {
    let fixture = TestCursorSnapshot::from_fixture();
    let snapshot_file = fixture.snapshot_string();
    let cursor_path = vec![fixture.main_tail, 1];
    let (node, revision) =
    ...
    assert_eq!(
      ...
      Some([fixture.main_tail, 2].as_slice())
    );
    assert_eq!(
  ...
  fn cursor_document_round_trips_named_cursors_history_stack_and_clipboard() {
    ...
    let inactive_path = vec![fixture.main_tail, 0];
  ...
  fn cut_and_paste_round_trip_tree_and_cursor_clipboard() {
    ...
    assert_eq!(cut.active.path, vec![fixture.main_tail]);
  ...
  fn cursor_follows_moved_subtree_even_when_fingerprint_is_duplicated() {
    ...
    let entry = snapshot
      ...
      .and_then(|file| file.defs.get_mut("main!"))
      .expect("fixture main definition should exist");
    let duplicated = apply_operation_at_path(&entry.code, &[fixture.main_tail, 1], TreeOperation::InsertAfter, Some(&node))
      .expect("duplicate should insert");
    let after_move_insert = apply_operation_at_path(&duplicated, &[fixture.main_tail, 0], TreeOperation::InsertAfter, Some(&node))
      .expect("move destination should insert");
    entry.code = apply_operation_at_path(&after_move_insert, &[fixture.main_tail, 2], TreeOperation::Delete, None)
      .expect("old source should delete");
    save_snapshot(&snapshot, &snapshot_file).expect("mutated fixture should save");
  ...
  fn cursor_navigation_supports_last_child_sibling_counts_and_multi_back() {
    let fixture = TestCursorSnapshot::from_fixture();
    let snapshot_file = fixture.snapshot_string();
    set_cursor_selection(&snapshot_file, "app.main/main!", vec![fixture.main_tail]).expect("cursor should select parent list");
    ...
    assert_eq!(
      ...
      vec![fixture.main_tail]
    );

    set_cursor_selection(&snapshot_file, "app.main/main!", vec![fixture.main_tail, 0]).expect("cursor should select first sibling");
