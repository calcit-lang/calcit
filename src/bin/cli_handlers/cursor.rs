...
mod tests {
  ...
  fn cut_and_paste_round_trip_tree_and_cursor_clipboard() {
    ...
    assert_eq!(cut.active.path, vec![42]);
  ...
  fn cursor_follows_moved_subtree_even_when_fingerprint_is_duplicated() {
    ...
    let entry = snapshot
      ...
      .expect("fixture main definition should exist");
    let duplicated =
      apply_operation_at_path(&entry.code, &[42, 1], TreeOperation::InsertAfter, Some(&node)).expect("duplicate should insert");
    let after_move_insert =
      apply_operation_at_path(&duplicated, &[42, 0], TreeOperation::InsertAfter, Some(&node)).expect("move destination should insert");
    entry.code = apply_operation_at_path(&after_move_insert, &[42, 2], TreeOperation::Delete, None).expect("old source should delete");
    save_snapshot(&snapshot, &snapshot_file).expect("mutated fixture should save");

    maintain_cursor_after_node_move(&snapshot_file, "app.main/main!", &[42, 1], &[42, 0], TreeOperation::InsertAfter, 0)
  ...
  fn cursor_navigation_supports_last_child_sibling_counts_and_multi_back() {
    let fixture = TestCursorSnapshot::from_fixture();
    let snapshot_file = fixture.snapshot_string();
    set_cursor_selection(&snapshot_file, "app.main/main!", vec![42]).expect("cursor should select parent list");
    ...
    set_cursor_selection(&snapshot_file, "app.main/main!", vec![42, 0]).expect("cursor should select first sibling");
