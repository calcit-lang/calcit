  ...
  match command.spawn() {
    Ok(mut child) => {
      ...
      ResponseJson(serde_json::json!({
          ...
          "tips": {
            "next_steps": [
              "Use 'generate_calcit_incremental' to apply code changes to the running process",
...
pub fn grab_calcit_runner_logs(_app_state: &super::AppState, _request: GrabCalcitRunnerLogsRequest) -> ResponseJson<Value> {
  ...
  let has_errors = log_entries
    .iter()
    .any(|entry| entry.get("is_error").and_then(|v| v.as_bool()).unwrap_or(false));
  ...
  ResponseJson(serde_json::json!({
      ...
      "tips": {
        "next_steps": if has_errors {
          ...
          vec![
            "Use 'generate_calcit_incremental' to apply code changes to the running process",
            "Continue making code edits as needed",
            "Use 'stop_calcit_runner' when done"
          ]
        },
