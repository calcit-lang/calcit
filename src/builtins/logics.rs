...
pub fn binary_equal(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (_, _) => CalcitErr::err_nodes(CalcitErrKind::Arity, "&= expected 2 arguments, but received:", xs),
  }
}
...
pub fn binary_less(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  if xs.len() == 2 {
    ...
    CalcitErr::err_nodes(CalcitErrKind::Arity, "&< expected 2 arguments, but received:", xs)
  }
}
...
pub fn not(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  if xs.len() != 1 {
    return CalcitErr::err_nodes(CalcitErrKind::Arity, "not expected a boolean or nil, but received:", xs);
  }
  match &xs[0] {
    Calcit::Nil => Ok(Calcit::Bool(true)),
    Calcit::Bool(b) => Ok(Calcit::Bool(!b)),
    a => CalcitErr::err_str(CalcitErrKind::Type, format!("not expected a boolean or nil, but received: {a}")),
  }
}
