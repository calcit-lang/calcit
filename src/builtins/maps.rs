...
pub fn call_new_map(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  if is_even(xs.len()) {
    ...
    CalcitErr::err_str(
      CalcitErrKind::Arity,
      format!("&{{}} expected even number of arguments, got: {}", CalcitList::from(xs)),
    )
  }
}
...
pub fn dissoc(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  if xs.len() < 2 {
    return CalcitErr::err_nodes(CalcitErrKind::Arity, "map dissoc expected at least 2 arguments:", xs);
  }
  match xs.first() {
    ...
    Some(a) => CalcitErr::err_str(CalcitErrKind::Type, format!("map dissoc expected a map, got: {a}")),
    _ => CalcitErr::err_nodes(CalcitErrKind::Arity, "map dissoc expected 2 arguments, got:", xs),
  }
}
...
pub fn get(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (Some(a), ..) => CalcitErr::err_str(CalcitErrKind::Type, format!("map &get expected map, got: {a}")),
    (None, ..) => CalcitErr::err_nodes(CalcitErrKind::Arity, "map &get expected 2 arguments, got:", xs),
  }
}
...
pub fn call_merge(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  if xs.len() == 2 {
    match (&xs[0], &xs[1]) {
        ...
        for (k, v) in ys {
          match k {
            Calcit::Str(s) | Calcit::Symbol { sym: s, .. } => match record.index_of(s) {
              ...
              None => return CalcitErr::err_str(CalcitErrKind::Type, format!("invalid field `{s}` for {fields:?}")),
            },
            Calcit::Tag(s) => match record.index_of(s.ref_str()) {
              Some(pos) => v.clone_into(&mut new_values[pos]),
              None => return CalcitErr::err_str(CalcitErrKind::Type, format!("invalid field `{s}` for {fields:?}")),
            },
            a => return CalcitErr::err_str(CalcitErrKind::Type, format!("invalid field key: {a}")),
          }
        }
      ...
      (a, b) => CalcitErr::err_str(CalcitErrKind::Type, format!("expected 2 maps, got: {a} {b}")),
    }
  } else {
...
pub fn to_pairs(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match xs.first() {
    ...
    Some(a) => CalcitErr::err_str(CalcitErrKind::Type, format!("to-pairs expected a map, got: {a}")),
    None => CalcitErr::err_str(CalcitErrKind::Arity, "to-pairs expected 1 argument, got nothing"),
  }
}
...
pub fn call_merge_non_nil(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (Some(a), Some(b)) => CalcitErr::err_str(CalcitErrKind::Type, format!("expected 2 maps, got: {a} {b}")),
...
pub fn to_list(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match xs.first() {
    ...
    Some(a) => CalcitErr::err_str(CalcitErrKind::Type, format!("&map:to-list expected a map, got: {a}")),
    None => CalcitErr::err_str(CalcitErrKind::Arity, "&map:to-list expected a map, got nothing"),
  }
}
...
pub fn count(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match xs.first() {
    ...
    Some(a) => CalcitErr::err_str(CalcitErrKind::Type, format!("map count expected a map, got: {a}")),
    None => CalcitErr::err_str(CalcitErrKind::Arity, "map count expected 1 argument"),
  }
}
...
pub fn empty_ques(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match xs.first() {
    ...
    Some(a) => CalcitErr::err_str(CalcitErrKind::Type, format!("map empty? expected some map, got: {a}")),
    None => CalcitErr::err_str(CalcitErrKind::Arity, "map empty? expected 1 argument"),
  }
}
...
pub fn contains_ques(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (Some(a), ..) => CalcitErr::err_str(CalcitErrKind::Type, format!("map contains? expected a map, got: {a}")),
    (None, ..) => CalcitErr::err_nodes(CalcitErrKind::Arity, "map contains? expected 2 arguments, got:", xs),
  }
}
...
pub fn includes_ques(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (Some(a), ..) => CalcitErr::err_str(CalcitErrKind::Type, format!("map `includes?` expected a map, got: {a}")),
    (None, ..) => CalcitErr::err_nodes(CalcitErrKind::Arity, "map `includes?` expected 2 arguments, got:", xs),
  }
}
...
pub fn destruct(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match xs.first() {
    ...
    Some(a) => CalcitErr::err_str(CalcitErrKind::Type, format!("&map:destruct expected a map, got: {a}")),
    None => CalcitErr::err_nodes(CalcitErrKind::Arity, "&map:destruct expected 1 argument, got:", xs),
  }
}
...
pub fn assoc(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match xs.first() {
    Some(Calcit::Map(base)) => {
      if xs.len() % 2 != 1 {
        CalcitErr::err_nodes(CalcitErrKind::Arity, "map:assoc expected odd number of arguments, got:", xs)
      } else {
        let size = (xs.len() - 1) / 2;
    ...
    Some(a) => CalcitErr::err_str(CalcitErrKind::Type, format!("map:assoc expected a map, got: {a}")),
    None => CalcitErr::err_nodes(CalcitErrKind::Arity, "map:assoc expected 3 arguments, got:", xs),
  }
}
...
pub fn diff_new(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (Some(a), Some(b)) => CalcitErr::err_str(CalcitErrKind::Type, format!("map:new_entries expected 2 maps, got: {a} {b}")),
    (..) => CalcitErr::err_nodes(CalcitErrKind::Arity, "map:diff-new expected 2 arguments, got", xs),
  }
}
...
pub fn diff_keys(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (Some(a), Some(b)) => CalcitErr::err_str(CalcitErrKind::Type, format!("map:diff-keys expected 2 maps, got: {a} {b}")),
    (..) => CalcitErr::err_nodes(CalcitErrKind::Arity, "map:diff-keys expected 2 arguments, got:", xs),
  }
}
...
pub fn common_keys(xs: &[Calcit]) -> Result<Calcit, CalcitErr> {
  match (xs.first(), xs.get(1)) {
    ...
    (Some(a), Some(b)) => CalcitErr::err_str(CalcitErrKind::Type, format!("map:diff-keys expected 2 maps, got: {a} {b}")),
    (..) => CalcitErr::err_nodes(CalcitErrKind::Arity, "map:common-keys expected 2 arguments, got:", xs),
  }
}
